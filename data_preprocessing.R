...
tackles_preprocess <- function(week, only_tacklers = T){
  ...
  week <- week %>% 
    mutate(mu_x = -s*sin((dir*pi/180))*0.5+x,
            mu_y = -s*cos((dir*pi/180))*0.5+y) %>% 
    left_join(ball_carrier, c('gameId', 'playId', 'frameId')) %>% 
    left_join(tackles, c('gameId', 'playId', 'nflId')) %>% 
  ...
  data_tackles <- week %>%
    ...
    ungroup() %>%
    group_by(gameId, playId, nflId) %>% 
    mutate(min_dist = ifelse(distance_to_ball == min(distance_to_ball), 1, 0)) %>% 
    ungroup() %>% 
    mutate(action =
             case_when(
    ...
    # mutate(dist_h = abs(x-x_car), dist_v = abs(y-y_car))
  ...
  #          distance_to_carrier_i = distance_to_ball, s_car_in = s_car,
  ...
  #   select(gameId, playId, nflId, frameId, s, a, o, dir, distance_to_ball, s_car, a_car,
  ...
  #          distance_to_ball = ifelse(is.na(distance_to_ball), 
  #                                       distance_to_carrier_i, distance_to_ball),
